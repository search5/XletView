...
//		
	...
	static String fixPath(String path) {		
		...
		Debug.info("fixPath incoming=" + path);
		
		result = path.replaceAll("\\\\", "\\/");
		result = result.replaceAll("\\//", "\\/");
		
		Debug.info("fixPath outgoing=" + result);
	...
	static String resolvePath(String parent, String child) {
		Debug.info("resolvePath(" + parent + ", " + child + ")");
	...
	static String resolveAbsolutePath(String parent, String child) {
		Debug.info("resolveAbsolutePath(" + parent + ", " + child + ")");
		String fullPath = "";
		...
		Debug.info("absolute path resolved");
		return result;
	}
//	
//	static String resolveParent(String path){
//		Debug.info("resolveParent(" + path + ")");
//		String result = "";
//		
...
//	}
	...
	int getBooleanAttributes(File f) {
		// TODO Auto-generated method stub
		Debug.info("getBooleanAttributes");
		return 0;
	}
	...
	long getLastModifiedTime(File f) {
		// TODO Auto-generated method stub
		Debug.info("getLastModifiedTime");
		return 0;
	}
	...
	boolean createFileExclusively(String pathname) throws IOException {
		// TODO Auto-generated method stub
		Debug.info("createFileExclusively");
		return false;
	}
	...
	boolean setLastModifiedTime(File f, long time) {
		// TODO Auto-generated method stub
		Debug.info("setLastModifiedTime");
		return false;
	}
	...
	static EmulatorFile getEmulatorFile(java.io.File file){
		...
		Debug.info("checking if the file " + absolutePath + " exists...");
		String[] names = absolutePath.split("\\" + separatorChar);
		
		if(currentMountPoint != null){
			Debug.info("currentMountPoint.getName()=" + currentMountPoint.getName());
		}
		...
		for (int i = 0; i < names.length; i++) {
			if(i < 1){
				Debug.info("skipping names[" + i + "] = _" + names[i] + "_");
				continue;
			}
			Debug.info("names[" + i + "] = _" + names[i] + "_");
			currentFile = currentFile.getChild(names[i]);
			

			if(currentFile != null){
				Debug.info(names[i] + " did exist");
			}
			else{
				Debug.info(names[i] + " did NOT exist");
				break;
			}
