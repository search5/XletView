...
//				addChild(new EmulatorFile(this, files[i]));
	...
	void addChild(EmulatorFile file){
		Debug.write(this,"addChild(" + file.getAbsolutePath() + ")");
...
//	}
	...
	EmulatorFile getChild(String name){
		...
		Debug.write(this,this.name + ", children.size() = " + children.size());
		for(int i = 0; i < children.size(); i++){
			EmulatorFile ef = (EmulatorFile) children.get(i);
			String s = ef.getName();
			Debug.write(this,"checking in " + getName() + " if there is a child with the name " + name + ", child has name " + s);
			if(s.equals(name)){
				result = ef;
				break;
			}
