...
public class HVisible extends HComponent implements HState{
    ...
    public HVisible(HLook hLook, int x, int y, int width, int height){
        ...
        contents = new Object[8];
        textContents = new String[8];
        graphicContents = new Image[8];
        animateContents = new Image[8][];
    ...
    */
    ...
    public void setTextContent(String string, int state){
    	...
    	if(state == HState.ALL_STATES){
    		for(int i = textContents.length - 1; i > -1 ; i--){
    			textContents[i] = string;
    		}
    	}
    	else{
    		textContents[state - HState.NORMAL_STATE] = string;
    	}
        callWidgetChanged();        
    }
    ...
    public void setGraphicContent(Image image, int state){
    	if(state == HState.ALL_STATES){
    		for(int i = graphicContents.length - 1; i > -1 ; i--){
    			graphicContents[i] = image;
    		}
    	}
    	else{
    		graphicContents[state - HState.NORMAL_STATE] = image;
    	}
    	callWidgetChanged();
    }
    ...
    public void setAnimateContent(Image[] imageArray, int state){
    	if(state == HState.ALL_STATES){
    		for(int i = animateContents.length - 1; i > -1 ; i--){
    			animateContents[i] = imageArray;
    		}
    	}
    	else{
    		animateContents[state - HState.NORMAL_STATE] = imageArray;
    	}
    	callWidgetChanged();
    }
    ...
    public void setContent(Object object, int state){
    	if(state == HState.ALL_STATES){
    		...
    		contents[state - HState.NORMAL_STATE] = object;
    	}
    	callWidgetChanged();
    }
    ...
    public String getTextContent(int state){
    	checkStateArgument(state);
    	String str = (String)getForNearestMatchingState(state, textContents);
        return str;
    }

    public Image getGraphicContent(int state){
    	checkStateArgument(state);
    	Image img = (Image) getForNearestMatchingState(state, graphicContents);
        return img;
    }

    public Image[] getAnimateContent(int state){
    	checkStateArgument(state);
    	Image[] imgs = (Image[])getForNearestMatchingState(state, animateContents);
    ...
    public Object getContent(int state){
    	checkStateArgument(state);
    	Object obj = getForNearestMatchingState(state, contents);
        return obj;
    }
    ...
    public Dimension getPreferredSize(){
        ...
        if(hLook != null){
            dimension = hLook.getPreferredSize(this);
        }
        else{
    ...
    public Dimension getMaximumSize(){
        Dimension dimension = null;
        if(hLook != null){
            dimension = hLook.getMaximumSize(this);
        }
        else{
            dimension = this.getSize();
        }
    ...
    public Dimension getMinimumSize(){
        Dimension dimension = null;
        if(hLook != null){
            dimension = hLook.getMinimumSize(this);
        }
        else{
            dimension = this.getSize();
        }
    ...
    public boolean isOpaque(){
        ...
        */
        if(backgroundMode == HVisible.NO_BACKGROUND_FILL){
            return false;
        }
        else{
            return true;
    ...
    public void setResizeMode(int resize){
        ...
        */
        if(resize != HVisible.RESIZE_NONE){
            ...
            System.out.println("setResizeMode(" + resize + ") is not supported, only HVisible.RESIZE_NONE is." + msg);
        }
    }
    ...
    */
    public void paint(Graphics g){
        if(hLook != null){
            hLook.showLook(g, this, this.getInteractionState());
        }
    }
